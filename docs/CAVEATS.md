...
- **Curves as chords**: curved HATCH edges, and the bulged segments of a HATCH polyline
  boundary, are drawn as polylines through sampled points. A SPLINE stored by control points, and a HATCH spline edge, is evaluated as the NURBS curve its degree,
  knots and weights define, and drawn through 16 points per knot span; one stored
...
- **Text codes are decoded to what they show, not drawn as formatting**: MTEXT paragraph
  ...
  colour, height, width) are dropped, and the `%%c`/`%%d`/`%%p` symbol codes in TEXT,
  ATTRIB and MTEXT become their characters. A `\U+XXXX` escape is the reader's to undo
  (the model's principles, §6.1), so the renderer draws what arrives: an escape a reader
  kept -- one naming an ASCII character -- is drawn as written. A format code that is not closed
...
- **Framing the changes** (`OverlayFrame::Changes`) shows the box of every cloud with half its longer side again around it -- cut to what the whole drawing's view shows, so it never shows more of the drawing than that -- drawn at the stroke width the whole render would have for a view that size (the whole render's stroke over its view box's diagonal, times the window's). A single changed point, a box with no extent, gets a window a fiftieth of the drawing's diagonal across. The original layer is then the same render written for that window.
