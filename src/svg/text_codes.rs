...
//!   left as written.
//! - **`\U+XXXX`**, the Unicode character, in TEXT and ATTRIB as in MTEXT: a
//!   file before R2007 writes a character outside its code page this way in
//!   any text, so a drawing saved both ways reads the same. Any other
//!   backslash in a TEXT or ATTRIB is a backslash.
...
pub(super) fn decode(text: &str, mtext: bool) -> String {
    ...
    while i < chars.len() {
        ...
        } else if c == '\\' && chars.get(i + 1) == Some(&'U') {
...
mod tests {
    ...
    fn percent_codes_read_the_same_in_a_text() {
        ...
        assert_eq!(decode("a%%009b", false), "a\tb");
