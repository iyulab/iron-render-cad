...
//! segment 1e6 drawing units long. That is a coordinate, not a length: in a
//! drawing a few thousandths of a unit across, or at a high pixel scale, the
//! segment lands billions of pixels off the canvas, and tiny-skia's
//! fixed-point scan converter panics on it.
//!
//! The right endpoints are the ones the picture's own edge cuts, and they
//! cannot be computed while the entity is rendered: the viewBox is only
//! settled once every entity has been walked. So the entity emits a
//! *placeholder* carrying the line -- the same trick
...
//! *written* in (y already flipped, and inside a block reference the
//! block's own space) together with [`InfiniteLine::matrix`], the
//! composition of the enclosing `<g transform>` matrices that takes those
//! coordinates to the document's. Clipping happens in document space, where
//! the viewBox lives; the endpoints come back as parameters along the line,
//! which an affine map preserves, so they are written back in the element's
//! own space without inverting anything.
...
/// likewise something no SVG this renderer emits otherwise contains.
const MARKER: &str = "@@IL@@";
...
pub(super) struct InfiniteLine {
    /// The element's own coordinates -> the document's: the enclosing
...
impl InfiniteLine {
    ...
    fn clip_to(&self, window: [f64; 4]) -> Option<((f64, f64), (f64, f64))> {
...
fn slab(
    ...
    if ![base.0, base.1, dir.0, dir.1, x0, y0, x1, y1]
        .iter()
        .all(|v| v.is_finite())
    {
        return None;
...
/// The document-space window an infinite line is clipped to: the viewBox
/// `(x, y, width, height)` as the document writes it, grown by a margin so
/// the stroke's cap and a dash's phase fall outside the picture rather than
/// ending at its edge.
///
/// A percent of the diagonal, which is far more than the stroke at any
/// normal width; four stroke widths when the stroke is the bigger of the two
/// (the automatic stroke width's 0.01 floor dwarfs a drawing a thousandth of
/// a unit across); and never more than one diagonal, so the coordinates stay
/// within a small multiple of the canvas whatever the caller asked for.
pub(super) fn window(x: f64, y: f64, width: f64, height: f64, stroke_width: f64) -> [f64; 4] {
...
pub(super) fn placeholder(line: &InfiniteLine, color: &str) -> String {
    ...
    // Eleven space-separated numbers and then the colour, which never
    // holds `@@`, taken as the whole rest of the payload.
    format!(
...
/// leaves of it, or with nothing when it shows none of it. Costs nothing for
/// a drawing with no construction lines, which is almost all of them.
pub(super) fn resolve(body: String, window: [f64; 4]) -> String {
    ...
    loop {
        ...
        let Some(end) = after.find("@@") else {
            // Malformed (never emitted) -- nothing of it may reach the
            // document.
            break;
        };
        if let Some((line, color)) = parse(&after[..end]) {
            if let Some((p0, p1)) = line.clip_to(window) {
                let _ = write!(
                    out,
...
mod tests {
    ...
    // The window every case below clips to: the square (10, 10) to (20, 20)
    // in document coordinates (y down).
    ...
    fn a_ray_starts_at_its_base_point_and_runs_only_forwards() {
        ...
        close(p0, (15.0, 15.0));
        close(p1, (20.0, 15.0));
        // Based outside and pointing in: it enters at x = 10.
        let (p0, p1) = top_level((0.0, 15.0), (1.0, 0.0), false)
            .clip_to(W)
            .expect("crosses the window");
        close(p0, (10.0, 15.0));
        close(p1, (20.0, 15.0));
        // Pointing away: nothing, while the same line as an XLINE crosses.
    ...
    #[test]
    fn a_line_that_misses_or_only_grazes_the_window_draws_nothing() {
        assert_eq!(top_level((15.0, 5.0), (1.0, 0.0), true).clip_to(W), None);
        assert_eq!(top_level((0.0, 30.0), (1.0, 1.0), true).clip_to(W), None);
        // Through the corner (10, 10) alone.
        assert_eq!(top_level((0.0, 20.0), (1.0, -1.0), true).clip_to(W), None);
        let zero = InfiniteLine {
            matrix: IDENTITY,
            base: (15.0, 15.0),
            dir: (0.0, 0.0),
            both_ways: true,
        };
        assert_eq!(zero.clip_to(W), None);
    }

    #[test]
    fn the_endpoints_come_back_in_the_lines_own_coordinates() {
        // Inside a group that scales by 2 and moves by (10, 10): own-space
        // (2.5, 2.5) is document (15, 15), and the window's vertical edges
        // at document x = 10 and 20 are own x = 0 and 5. Composing the same
        // group onto an identity line gives the same answer.
        let group = [2.0, 0.0, 0.0, 2.0, 10.0, 10.0];
        for matrix in [group, compose(group, IDENTITY)] {
    ...
    #[test]
    fn a_placeholder_round_trips_and_resolves_in_place() {
        let line = InfiniteLine {
        ...
        assert!(out.contains("<line x1=\"0\"/>") && out.contains("<circle/>"));
        assert!(
            out.contains("<line x1=\"10\" y1=\"15\" x2=\"20\" y2=\"15\" stroke-dasharray=\"4,2\" stroke=\"black\"/>"),
            "{out}"
        );
        // A line the window misses leaves nothing behind.
        let body = format!(
            "before{}after",
        ...
        assert_eq!(resolve("<line/>".to_string(), W), "<line/>");
    }

    #[test]
    fn the_window_grows_the_view_box_by_a_bounded_margin() {
        let margin = 100.0_f64.hypot(100.0) * 0.01;
        let [x0, y0, x1, y1] = window(0.0, -100.0, 100.0, 100.0, 0.0);
        ...
        let [x0, ..] = window(0.0, -100.0, 100.0, 100.0, 10.0);
        assert!((x0 + 40.0).abs() < 1e-12, "{x0}");
        // ...but never past one diagonal.
        let [x0, ..] = window(0.0, -100.0, 100.0, 100.0, 1e6);
