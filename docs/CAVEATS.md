...
- **Curves as chords**: curved HATCH edges, and the bulged segments of a HATCH polyline
  ...
  foreshortening also narrows the strokes across it. A HATCH's fill style is not reproduced: every
  hatch is filled alternating from the outside in, which is the normal style; "outermost only" and
  "ignore islands" need to know which path is outermost, which the model does not carry. A TEXT or ATTRIB in its own plane is drawn the same way (a mirror
